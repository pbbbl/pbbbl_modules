...
function clean(
  ...
  const transformEach =
    isDefined(config.transformEach) && isFunction(config.transformEach)
      ? config.transformEach
      : null;
  const og = deepFreeze(
    copy({
      root: object,
  ...
  return transform(object, (result, value, key) => {
    ...
    const configIsUndefined =
      isUndefined(config.isUndefined) ||
      (isDefined(config.isUndefined) &&
        (isTrue(config.isUndefined) || config.isUndefined !== false));
    const configIsFunction =
      isUndefined(config.isFunction) ||
      (isDefined(config.isFunction) &&
        (isTrue(config.isFunction) || config.isFunction !== false));
    const configIsError =
      isUndefined(config.isError) ||
      (isDefined(config.isError) &&
        (isTrue(config.isError) || config.isError !== false));
    const configIsEmpty = isDefined(config.isEmpty) && isTrue(config.isEmpty);
    const configIsEmptyArray =
      configIsEmpty ||
      (isDefined(config.isEmptyArray) && isTrue(config.isEmptyArray));
    const configIsEmptyObject =
      configIsEmpty ||
      (isDefined(config.isEmptyObject) && isTrue(config.isEmptyObject));
    const configIsEmptyString =
      configIsEmpty ||
      (isDefined(config.isEmptyString) && isTrue(config.isEmptyString));
    const isRecursable =
      isDefined(value) &&
    ...
    if (isDefined(transformEach) && isFunction(transformEach)) {
      ...
      const current = isArray(result)
        ? [...result]
        : isObject(result)
        ? { ...result }
        : result;
      const transformArgs = deepFreeze([
        ...copy([value, key]),
        { current, ...og },
      ]);
