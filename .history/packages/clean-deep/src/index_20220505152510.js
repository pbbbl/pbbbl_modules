...
function createOptions(config = configs._default) {
    ...
    const rootConfig = configs[rootConfigId];
    let output = { ...rootConfig, ...config };
    return output;
//   const configKeys = Object.keys({...configs._default, ...rootConfig, ...config});
//     configKeys.forEach(key => {
//     if (isDefined(config[key]) && isType(config[key], typeof rootConfig[key])) {
