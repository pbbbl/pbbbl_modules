...
function clean(valueToClean, options = { ...defaultOptions }, isRecurse=false) {
  ...
  return transform(object, (result, value, key) => {
    ...
    const keys =
      isDefined(config.keys) && config.keys && !isEmptyArray(config.keys)
        ? config.keys
        : false;
    ...
    if (
      ...
      (isObject(value) && !isEmptyObject(value) && !isNull(value))
    ) {

        console.log('recurse @',{key,value});
        value = clean(value, options, true);
    }
    const values =
      isDefined(config.values) && config.values && !isEmptyArray(config.values)
        ? config.values
        : false;
    // Exclude specific values.
    if (values && config.values.includes(value)) {
        console.log('remove value',{key,value});
      return;
    }
    ...
    const emptyValues = isDefined(config.isEmpty) && config.isEmpty === true;
    ...
    if (isObject(value) && emptyObjects && isEmptyObject(value)) {
        console.log('remove emptyObject',{key,value});
    ...
    const emptyArrays =
      emptyValues ||
      (isDefined(config.isEmptyArray) && config.isEmptyArray === true);
    if (isArray(value) && emptyArrays && isEmptyArray(value)) {
        console.log('remove array',{key,value});
      // console.log("emptyObjects", { value });
      return;
    }
    ...
    const emptyStrings =
      emptyValues ||
      (isDefined(config.isEmptyString) && config.isEmptyString === true);
    if (isString(value) && emptyStrings && isEmptyString(value)) {
        console.log('remove emptyString',{key,value});
      return;
    }
    ...
    if (NaNValues && isNaN(value)) {
        console.log('remove NaN',{key,value});
      return;
    }

    // Exclude functions.
    const functions = typeof config.isFunction === 'undefined' ? true :
      isDefined(config.isFunction) && config.isFunction === true ? true : false;
    if (functions && typeof value == 'function') {
        console.log('remove function',{key,value});
    ...
    if (nullValues && value === null) {
            console.log('remove null',{key,value});
      // console.log("nullValues", { value });
      return;
    }

    // Exclude undefined values.
    const undefinedValues = typeof config.isUndefined === 'undefined' || (
      isDefined(config.isUndefined) && config.isUndefined === true);
    if (undefinedValues && value === undefined) {
        console.log('remove undefined',{key,value});
        
      // console.log("undefined", { value: value || undefined });
      return;
    }
    // Exclude error values.
    const errorValues = typeof config.isError === 'undefined' || (
        isDefined(config.isError) && config.isError === true);
    if (errorValues && value instanceof Error) {
        console.log('remove error',{key,value});
        return;
    }
