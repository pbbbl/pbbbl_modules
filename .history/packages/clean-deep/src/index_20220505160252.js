...
const deepFreeze = requier('deep-freeze')
...
function clean(valueToClean, options = { ...defaultOptions }, isRecurse=false) {
  ...
  return transform(object, (result, value, key) => {
    ...
    if(isDefined(transformEach) && isFunction(transformEach)){
        const ogResult = deepFreeze(copy(result));
        const transformArgs = deepFreeze([...copy([value,key]),{result:ogResult,root:og.object,config:og.config}]);
        const transformed = transformEach(value,key,result,object,config);
